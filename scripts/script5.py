...
##открываем таблицу
##Лидер platinum (сайт заказчика) 
url_lider_plat = "https://artvod.ru/catalog/lider-platinum-19/" ##ссылка на сайт
pages = requests.get(url_lider_plat).text ##получаем html
...
df.loc[7, current_date] = float(''.join([i for i in third if i.isdigit()]))
df.loc[8, current_date] = float(''.join([i for i in four if i.isdigit()]))
##сохраняем таблицу
